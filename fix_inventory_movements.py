#!/usr/bin/env python3
# fix_inventory_movements.py
# يضيف نموذج StockMovement + الـ API + تاب حركة المخزون في الـ UI
...
FRONTEND = BASE / 'pos_frontend' / 'src' / 'pages' / 'InventoryPage.jsx'
...
# ─── 5. FRONTEND ──────────────────────────────────────────────────────────────
FRONTEND_JSX = r'''import React, { useCallback, useEffect, useState } from 'react';
...
print('\n=== fix_inventory_movements.py ===\n')
write(BACKEND / 'models.py',      MODELS)
write(BACKEND / 'serializers.py', SERIALIZERS)
write(BACKEND / 'views.py',       VIEWS)
write(BACKEND / 'urls.py',        URLS)
write(FRONTEND,                   FRONTEND_JSX)

# ─── 7. PATCH api.js ──────────────────────────────────────────────────────────
if API_FILE.exists():
    content = API_FILE.read_text(encoding='utf-8')
    if 'getMovements' not in content:
        old = 'resolveAlert: (id)'
        new = ('getMovements:     (params={}) => api.get(\'/inventory/movements/\', { params }),\n'
               '  resolveAlert: (id)')
        if old in content:
            content = content.replace(old, new)
            API_FILE.write_text(content, encoding='utf-8')
            print('  [ok] api.js patched with getMovements')
        else:
            print('  [warn] api.js: could not find insertion point — add getMovements manually')
    else:
        print('  [skip] api.js already has getMovements')
else:
    print(f'  [warn] {API_FILE} not found — skip api.js patch')

# ─── 8. MIGRATIONS ────────────────────────────────────────────────────────────
print('\n--- running migrations ---')
manage = BASE / 'pos_backend' / 'manage.py'
if manage.exists():
    subprocess.run(['python', str(manage), 'makemigrations', 'inventory'], cwd=BASE/'pos_backend')
    subprocess.run(['python', str(manage), 'migrate'],                    cwd=BASE/'pos_backend')
else:
    print('[warn] manage.py not found — run migrations manually')

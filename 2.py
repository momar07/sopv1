#!/usr/bin/env python3
"""
fix_operations_page.py
======================
يكتب ملفين:
  1. pos_frontend/src/pages/Operations.jsx       — صفحة محسّنة كاملة
  2. pos_frontend/src/pages/OperationDetails.jsx — بدون تغيير (backup فقط)

التحسينات:
  ✅ إصلاح canCancel bug (isAdmin/isManager functions)
  ✅ سيرش دايم ظاهر + فلتر تاريخ + فلتر طريقة الدفع
  ✅ Summary cards (إجمالي اليوم، عدد الفواتير، المرتجعات، الملغاة)
  ✅ عمود "مرتجع" في الجدول ببadge برتقالي
  ✅ عمود "الكاشير" في الجدول
  ✅ عمود "طريقة الدفع" في الجدول
  ✅ Pagination بسيط (السابق / التالي)
  ✅ زرار الإلغاء ظاهر دايماً للمدير بدون قيد التبويب
"""
...
FILES = {
    'operations': os.path.join(FRONT, 'pages', 'Operations.jsx'),
}
...
# ══════════════════════════════════════════════════════════════════════
print('\n=== fix_operations_page.py ===\n')
write(FILES['operations'], OPERATIONS_CONTENT)
print("""
✅ تم بنجاح!

ملاحظة: الـ backend لازم يدعم الـ params دي في /api/sales/:
  - search
  - status
  - payment_method
  - start_date / end_date  ← لو مش موجودين راجع views.py

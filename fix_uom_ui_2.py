# fix_products_uom.py
"""
يكتب Products.jsx كامل مع إضافة تبويب الوحدات والأسعار
بدون أي خطوات يدوية
"""
...
PRODUCTS = os.path.join(FRONTEND, "pages/Products.jsx")
...
# ── 1. أضف unitsAPI في api.js لو مش موجود ─────────────────────────────────
def patch_api_js():
    with open(API_FILE, "r", encoding="utf-8") as f:
        content = f.read()
    if "unitsAPI" in content:
...
"""
    content = content.replace("export default api;", block + "\nexport default api;")
    with open(API_FILE, "w", encoding="utf-8") as f:
        f.write(content)
    print("  ✅ unitsAPI أُضيف في api.js")

# ── 2. Products.jsx الكامل ────────────────────────────────────────────────
PRODUCTS_JSX = r"""
...
const useToast = () => {
  ...
  const ToastContainer = useCallback(() => (
    <div className="fixed bottom-5 left-5 z-[9999] flex flex-col gap-2 max-w-sm">
      {toasts.map(t => (
        <div key={t.id} className={`flex items-start justify-between gap-3 px-4 py-3 rounded-xl shadow-lg border font-semibold text-sm animate-fadeIn
  ...
  return (
    <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-3 py-2 w-fit">
      ...
      <button onClick={apply} disabled={loading}
        ...
        {loading?<i className="fas fa-circle-notch fa-spin"/>:'تطبيق'}
      </button>
      ...
      <div>
        ...
        {isEdit && unitPrices.length===0 && (
          <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-500 text-center">
            ...
            لا توجد وحدات مضافة — اختر وحدة من القائمة أعلاه.
          </div>
        )}
            ...
            {unitPrices.map(up=>(
              <div key={up.unit}
                ...
                <div className="col-span-2 text-center">
                  <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-lg font-mono">×{up.factor}</span>
                </div>
                <div className="col-span-3">
                  <input type="number" step="0.01" min="0"
                    ...
                    className={`w-full border rounded-lg px-2 py-1 text-sm text-center font-bold outline-none transition
                      ${up.is_auto?'bg-gray-100 text-gray-500 border-gray-200 cursor-not-allowed':'border-blue-300 focus:ring-2 focus:ring-blue-100 text-blue-700'}`}/>
                </div>
                ...
                <div className="col-span-1 flex justify-center">
                  <input type="checkbox" checked={up.is_active}
                    onChange={e=>onPriceChange(up.unit,'is_active',e.target.checked)}
                    className="w-4 h-4 accent-blue-600 cursor-pointer"/>
                </div>
                <div className="col-span-1 flex justify-center">
                  <button type="button" onClick={()=>onRemovePrice(up.unit)}
                    className="text-red-400 hover:text-red-600 transition text-sm" title="حذف">
...
const EMPTY_FORM = {
  ...
  base_unit: null, purchase_unit: null,
};
...
const ProductModal = ({ open, product, categories, units, onClose, onSaved, toast }) => {
  ...
  const handlePriceChange = (unitId, field, val) => {
    setUnitPrices(prev => prev.map(up =>
      up.unit === unitId ? { ...up, [field]: val } : up
    ));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // حفظ أسعار الوحدات لو فيه
      if (unitPrices.length > 0 && savedProduct?.id) {
        setSavingP(true);
        try {
          await unitsAPI.setUnitPrices(savedProduct.id, unitPrices);
        } catch {
          toast('warning', '⚠️ تم حفظ المنتج لكن حدث خطأ في أسعار الوحدات');
        } finally {
          setSavingP(false);
        }
      }
            ...
            <div className="space-y-4">
              ...
              {liveMargin!==null && (
                <div className={`flex items-center gap-3 p-3 rounded-xl border text-sm font-bold
                  ...
                  {priceWarn?'⚠️ تحذير: السعر أقل من التكلفة! ستبيع بخسارة.':`هامش الربح: ${liveMargin}%`}
                </div>
              )}
              <div>
                ...
                {product && (
                  <p className="text-xs text-blue-600 mt-1">
                    ...
                    الكمية الحالية: <strong>{product.stock}</strong>
                  </p>
                )}
              </div>
              <div>
...
const Products = () => {
  ...
  // ── Fetch units + categories مرة واحدة ────────────────────────────────
  useEffect(() => {
    ...
    unitsAPI.getAll({ page_size:200 })
      ...
      .catch(() => toast('warning','⚠️ تعذّر تحميل وحدات القياس'));
  }, []);
  ...
  useEffect(() => { setPage(1); fetchProducts({ pg:1 }); },
  ...
  const handleBulkDelete = () => setConfirmState({
    open:true, title:'حذف متعدد',
    message:`هل أنت متأكد من حذف ${selected.size} منتج؟`,
    ...
    onConfirm: async () => {
      ...
      try {
        ...
        toast('success',`✅ تم حذف ${selected.size} منتج`);
    ...
    onConfirm: async () => {
      ...
      try {
        await Promise.all([...selected].map(id=>{
          ...
          return productsAPI.update(id,{...p,is_active:activate});
        }));
  ...
  const handleDelete = p => setConfirmState({
    open:true, title:'حذف منتج', message:`هل أنت متأكد من حذف "${p.name}"؟`,
                ...
                <tr key={product.id}
                  ...
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={selected.has(product.id)}
                      onChange={()=>toggleSelect(product.id)} className="w-4 h-4 accent-blue-600"/>
                  </td>
                      ...
                      <div>
                        ...
                        {product.barcode && (
                          <button onClick={()=>copyBarcode(product.barcode)}
                            ...
                            <i className="fas fa-barcode"></i>{product.barcode}
                          </button>
                        )}
                      </div>
                    </div>
                  ...
                  <td className="px-4 py-3">
                    {product.base_unit_name?(
                      <span className="px-2 py-1 rounded-full text-xs font-bold bg-purple-50 text-purple-700">
                        <i className="fas fa-ruler ml-1"></i>{product.base_unit_name}
                      </span>
                    ):<span className="text-gray-400 text-xs">—</span>}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {adjustingId===product.id?(
                      <InlineStockAdjust product={product} onDone={handleStockDone}
                        onError={msg=>toast('error',`❌ ${msg}`)}/>
                    ):(
                      <div className="flex items-center gap-2">
                        <StockBadge stock={product.stock}/>
                        <button onClick={()=>setAdjustingId(product.id)}
                          className="text-xs text-blue-600 hover:text-blue-800 font-bold transition" title="تعديل المخزون">
        ...
        {totalPages>1 && (
          <div className="px-6 py-4 border-t flex items-center justify-between flex-wrap gap-3">
            <span className="text-sm text-gray-500">
              عرض {((page-1)*pageSize)+1} – {Math.min(page*pageSize,total)} من {total} منتج
            </span>
            <div className="flex gap-2">
              <button onClick={()=>setPage(1)} disabled={page===1}
                className="px-3 py-1.5 rounded-lg border text-sm font-bold disabled:opacity-40 hover:bg-gray-50 transition">
                «
              </button>
                ...
                return (
                  <button key={pg} onClick={()=>setPage(pg)}
                    className={`px-3 py-1.5 rounded-lg border text-sm font-bold transition
                      ${page===pg?'bg-blue-600 text-white border-blue-600':'hover:bg-gray-50'}`}>
                    {pg}
                  </button>
                );
              })}
...
print()
print("✅ تم! شغّل:")
print("   python3 fix_products_uom.py")

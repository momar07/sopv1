  ...
  return (
    <div dir="rtl" className="p-4 min-h-screen bg-gray-50">
      <div className="mb-5">
        ...
        <p className="text-gray-500 text-sm mt-1">تنبيهات المخزون · أوامر الشراء · استلام البضاعة</p>
      </div>
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${alert.alert_type==='out'?'bg-red-100 text-red-700':'bg-yellow-100 text-yellow-700'}`}>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={load} className="bg-white border text-gray-600 hover:bg-gray-50 font-bold px-3 py-1.5 rounded-xl text-sm">
            🔄 تحديث
          </button>
      ...
      {alerts.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center text-gray-400">
          ...
          <p className="font-bold">لا توجد تنبيهات في هذا التصفية</p>
        </div>
      ) : (
...
function AlertTicketModal({ alert, suppliers, onClose, onUpdated, notify }) {
  ...
  const handleCreatePO = async () => {
    ...
    try {
      ...
      notify('✅ تم إنشاء أمر الشراء وربطه بالتنبيه');
      onUpdated();
    } catch(e) { notify('خطأ: '+(e?.response?.data?.error||JSON.stringify(e?.response?.data)||'خطأ'), 'error'); }
    finally { setSaving(false); }
  };
  ...
  return (
    <Modal title={`تذكرة: ${data.product_name}`} onClose={onClose}>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <span className={`text-xs font-bold px-2 py-1 rounded-full ${data.alert_type==='out'?'bg-red-100 text-red-700':'bg-yellow-100 text-yellow-700'}`}>
          {data.alert_type==='out'?'🚨 نفاد المخزون':'⚠️ مخزون منخفض'}
        </span>
            ...
            data.notes.map(n => {
              ...
              return (
                <div key={n.id} className={`rounded-xl border p-3 ${nc.color}`}>
                  ...
                  {n.cost && <p className="text-xs text-green-700 mt-1 font-bold">💰 التكلفة: {fmt(n.cost)} ج.م</p>}
        ...
        <div className="space-y-3">
          <Field label="نوع الملاحظة">
            <select className={INP} value={noteForm.note_type} onChange={e=>setNote({...noteForm,note_type:e.target.value})}>
              {Object.entries(noteTypeConfig).map(([k,v]) => <option key={k} value={k}>{v.icon} {v.label}</option>)}
            </select>
          </Field>
          <Field label="النص *">
            <textarea className={INP} rows={3} value={noteForm.text} onChange={e=>setNote({...noteForm,text:e.target.value})} placeholder="اكتب ملاحظتك هنا..." />
          </Field>
          {noteForm.note_type === 'quote' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <Field label="التكلفة">
                  <input type="number" className={INP} value={noteForm.cost} onChange={e=>setNote({...noteForm,cost:e.target.value})} placeholder="0.00" />
                </Field>
                <Field label="اسم المورد">
                  <input type="text" className={INP} value={noteForm.supplier_name} onChange={e=>setNote({...noteForm,supplier_name:e.target.value})} placeholder="اسم المورد" />
                </Field>
              </div>
              <Field label="تاريخ الاستلام المتوقع">
                <input type="date" className={INP} value={noteForm.expected_date} onChange={e=>setNote({...noteForm,expected_date:e.target.value})} />
              </Field>
            </>
          )}
          {noteForm.note_type === 'delay' && (
            <Field label="سبب التأخير">
              <textarea className={INP} rows={2} value={noteForm.delay_reason} onChange={e=>setNote({...noteForm,delay_reason:e.target.value})} placeholder="اشرح سبب التأخير..." />
            </Field>
          )}
          ...
          {data.linked_po_reference ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="الكمية *">
                  <input type="number" min={1} className={INP} value={poForm.quantity} onChange={e=>setPo({...poForm,quantity:e.target.value})} />
                </Field>
                <Field label="تكلفة الوحدة *">
                  <input type="number" step="0.01" className={INP} value={poForm.unit_cost} onChange={e=>setPo({...poForm,unit_cost:e.target.value})} />
                </Field>
                <Field label="تاريخ الاستلام المتوقع">
                  <input type="date" className={INP} value={poForm.expected_date} onChange={e=>setPo({...poForm,expected_date:e.target.value})} />
                </Field>
              </div>
              <Field label="ملاحظات">
                <input type="text" className={INP} value={poForm.notes} onChange={e=>setPo({...poForm,notes:e.target.value})} placeholder="اختياري" />
              </Field>
      ...
      {tab === 'resolve' && (
        <div className="space-y-3">
          {data.is_resolved ? (
            ...
            <>
              {data.linked_po_reference && data.linked_po_status !== 'received' && (
                <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 text-sm text-orange-700 font-bold">
                  ⚠️ أمر الشراء {data.linked_po_reference} لم يُستلم بعد. تأكد من الاستلام أولاً.
                </div>
              )}
